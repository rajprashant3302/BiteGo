...
import { ChevronLeft, ShoppingBag, Leaf, Flame, Star } from 'lucide-react';
...
function toCartItem(item: SearchMenuItem) {
  return {
    ItemID: item.id,
    ItemName: item.name,
    Price: item.price,
...
/** CartContext is untyped JSX; useContext infers `null` only without this. */
type CartActions = { addToCart: (item: ReturnType<typeof toCartItem>) => void };

export default function MenuItemPage() {
  const params = useParams();
  const router = useRouter();
  const cart = useCart() as CartActions | null;
  const [item, setItem] = useState<SearchMenuItem | null>(null);
    ...
    const fetchItem = async () => {
      try {
        ...
        const found = (json.data || []).find(
          (i: SearchMenuItem) =>
            String(i.id) === String(id) && i.type !== 'RESTAURANT'
        );
        setItem(found || null);
      } catch (err) {
  ...
  if (loading)
    return (
      <div className="h-screen flex items-center justify-center font-black animate-pulse text-orange-500 uppercase tracking-widest">
        Preparing your bite...
      </div>
    );
  if (!item)
        ...
        >
          ...
          {item.isVeg && (
            <div className="absolute top-6 left-6 bg-white/90 backdrop-blur-md px-4 py-2 rounded-2xl flex items-center gap-2 shadow-lg">
              ...
              <span className="text-xs font-black text-green-600 uppercase tracking-tighter">
                100% Veg
              </span>
            </div>
          )}
