    ...
    async getPaymentById(_, { id }, { userId }) {
      try {
        ...
        if (payment.UserID !== userId) {
          throw new UnauthorizedError('Cannot access this payment');
        }
  ...
  Mutation: {
    ...
    async makePayment(_, { orderId, input }, { userId, userRole }) {
      try {
        ...
        if (paymentMethod === 'Wallet') {
          ...
          if (new Decimal(order.user.WalletBalance).lessThan(new Decimal(amount))) {
          ...
          await prisma.user.update({
            ...
            data: {
              WalletBalance: new Decimal(order.user.WalletBalance).minus(new Decimal(amount)),
            },
          });
        }
    ...
    async refundPayment(_, { paymentId, reason }, { userId, userRole }) {
      try {
        ...
        await prisma.user.update({
          where: { UserID: payment.UserID },
          data: {
            WalletBalance: new Decimal(user.WalletBalance).plus(new Decimal(payment.TotalAmount)),
          },
        });
    ...
    async refundOrder(_, { orderId, reason }, { userId, userRole }) {
      try {
        ...
        await prisma.user.update({
          where: { UserID: order.UserID },
          data: {
            WalletBalance: new Decimal(order.user.WalletBalance).plus(new Decimal(successfulPayment.TotalAmount)),
          },
        });
    ...
    async payWithWallet(_, { orderId }, { userId, userRole }) {
      try {
        ...
        if (new Decimal(order.user.WalletBalance).lessThan(new Decimal(order.TotalAmount))) {
          throw new ValidationError('Insufficient wallet balance');
        }
        ...
        const updatedUser = await prisma.user.update({
          where: { UserID: order.UserID },
          data: {
            WalletBalance: new Decimal(order.user.WalletBalance).minus(new Decimal(order.TotalAmount)),
          },
        });
