...
const userResolvers = {
  Query: {
    ...
    async getUserByEmail(_, { email }, context) {
      try {
        requireRole(context, [...ADMIN_ROLES, ...SUPPORT_ROLES]);
        validateEmail(email);
     ...
     * Get user addresses
     */
    async getUserAddresses(_, { userId }, context) {
      try {
     ...
     * Get address by ID
     */
    async getAddressById(_, { id }, context) {
      try {
        requireAuth(context);
        ...
        requireSelfOrAdmin(context, address.UserID);
     ...
     */
    async getUserWalletBalance(_, { userId }, context) {
      try {
        requireSelfOrAdmin(context, userId);
        const user = await prisma.user.findUnique({
          where: { UserID: userId },
     ...
     */
    async getUserWalletTransactions(_, { userId, limit = 50, offset = 0 }, context) {
      try {
        requireSelfOrAdmin(context, userId);
  ...
  Mutation: {
    ...
    async addWalletBalance(_, { userId, amount }, { userId: authUserId, userRole }) {
      try {
        requireRole({ userId: authUserId, userRole }, ADMIN_ROLES);
        const user = await prisma.user.findUnique({
          where: { UserID: userId },
        });
        ...
        const newBalance = new Decimal(user.WalletBalance).plus(new Decimal(amount));
    ...
    async deductWalletBalance(_, { userId, amount, description }, { userId: authUserId }) {
      try {
        ...
        if (new Decimal(user.WalletBalance).lessThan(new Decimal(amount))) {
          throw new ValidationError('Insufficient wallet balance');
        }

        const newBalance = new Decimal(user.WalletBalance).minus(new Decimal(amount));
